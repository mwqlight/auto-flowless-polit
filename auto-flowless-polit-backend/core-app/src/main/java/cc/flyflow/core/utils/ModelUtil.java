...
import cc.flyflow.common.dto.flow.node.DelayNode;
...
import cc.flyflow.core.expression.condition.NodeExpressionStrategyFactory;
import cc.flyflow.core.listeners.AllEventListener;
import cc.flyflow.core.listeners.ApprovalCreateListener;
import cc.flyflow.core.listeners.StarterUserTaskCreateListener;
import cc.flyflow.core.node.IDataStoreHandler;
import cc.flyflow.core.node.NodeDataStoreFactory;
import cc.flyflow.core.servicetask.ApproveServiceTask;
import cc.flyflow.core.servicetask.CopyServiceTask;
...
import static cc.flyflow.common.constants.ProcessInstanceConstant.MERGE_GATEWAY_FLAG;
    ...
    private static IntermediateCatchEvent buildDelayNode(Node node) {
        ...
        if (delayNode.getMode()) {
            if (delayNode.getDelayUnit().length() == 1) {
                //年月日
                timerEventDefinition.setTimeDuration(StrUtil.format("P{}{}", delayNode.getValue().toString(), delayNode.getDelayUnit()));
            } else {
                //时分秒
                timerEventDefinition.setTimeDuration(StrUtil.format("PT{}{}", delayNode.getValue().toString(),
